(function(){
  ...
  function config(step,interactive,request,withActions=false,history=false){
  ...
  function render(row,options={}){
    ...
    if(!options.container&&row&&state.plan?.version===row.version&&box.dataset.planVersion===renderKey)return;
