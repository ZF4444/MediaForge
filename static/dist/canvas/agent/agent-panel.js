(function(){
  ...
  function renderRun(data) {
    ...
    let pending=[]; let turnOpen=false; let currentPlanRendered=false;
    timeline.forEach(entry=>{
      ...
      if(entry.kind==='plan'){
        if(data.plan && String(entry.item.version||'')===String(data.plan.version||'')){
          messages.appendChild(planBox); window.CanvasAgentPlan.render(data.plan,{interactive:run.status==='awaiting_confirmation'&&!confirmationAccepted,status:planStates.get(String(entry.item.version||''))||''}); currentPlanRendered=true; return;
        }
        ...
        messages.appendChild(snapshot); window.CanvasAgentPlan.render(entry.item,{interactive:false,container:snapshot,status:planStates.get(String(entry.item.version||''))||''}); return;
      }
    ...
    if(data.plan && !currentPlanRendered){ messages.appendChild(planBox); window.CanvasAgentPlan.render(data.plan,{interactive:run.status==='awaiting_confirmation'&&!confirmationAccepted,status:planStates.get(String(data.plan.version||''))||''}); }
    else if(!currentPlanRendered) { planBox.innerHTML=''; planBox.hidden=true; messages.appendChild(planBox); }
