...
def test_resource_gateway_resolves_and_governs_workflow(monkeypatch):
    target = ResolvedTarget(Connection("c1", "legacy", "comfyui_workflow", "Comfy", "", True), resource=ExecutableResource("r1", "c1", "comfyui_workflow", "wf"))
    seen = {}
    ...
    def no_governance(*args, **kwargs):
        ...
        return Context()
    monkeypatch.setattr("app.ai.gateway.provider_operation", no_governance)
